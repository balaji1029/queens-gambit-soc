...
class Engine:
    ...
    def __init__(self, fen='rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq'):
        ...
        self.transposition_table = json.load(open('transposition_table.json', 'r'))
        ...
        for i in range(64):
            ...
            if piece is not None:
                ...
                if piece_type.isupper():
                    ...
                    if piece_type == 'P':
                        white_eval += 1.0
                        white_eval += self.pawns_util[i] / 100
                        white_pawns += 1
                    elif piece_type == 'N':
                        white_eval += 3.0
                        white_eval += self.knights_util[i] / 100
                    elif piece_type == 'B':
                        white_eval += 3.25
                        white_eval += self.bishops_util[i] / 100
                        white_bishop_count += 1
                    elif piece_type == 'R':
                        white_eval += 5.0
                        white_eval += self.rooks_util[i] / 100
                    elif piece_type == 'Q':
                        white_eval += 9.75
                        white_eval += self.queens_util[i] / 100
                    elif piece_type == 'K':
                        if (white_pieces + black_pieces) > 8:
                            white_eval += self.king_start_util[i] / 100
                        else:
                            white_eval += self.king_end_util[i] / 100
                else:
                    black_pieces += 1
                    if piece_type == 'p':
                        black_eval += 1.0
                        black_eval += self.pawns_util[chess.square_mirror(i)] / 100
                        black_pawns += 1
                    elif piece_type == 'n':
                        black_eval += 3.0
                        black_eval += self.knights_util[chess.square_mirror(i)] / 100
                    elif piece_type == 'b':
                        black_eval += 3.25
                        black_eval += self.bishops_util[chess.square_mirror(i)] / 100
                        black_bishop_count += 1
                    elif piece_type == 'r':
                        black_eval += 5.0
                        black_eval += self.rooks_util[chess.square_mirror(i)] / 100
                    elif piece_type == 'q':
                        black_eval += 9.75
                        black_eval += self.queens_util[chess.square_mirror(i)] / 100
                    elif piece_type == 'k':
                        if (white_pieces + black_pieces) > 8:
                            black_eval += self.king_start_util[chess.square_mirror(i)] / 100
                        else:
                            black_eval += self.king_end_util[chess.square_mirror(i)] / 100
    ...
    def move_order(self, moves: list) -> list:
        ...
        for move in moves:
            ...
            if self.board.is_checkmate():
                move_list.append((math.inf, move))
    ...
    def alpha_beta_pruning(self, depth: int, alpha: float, beta: float) -> tuple[float, str]:
        ...
        for move in ordered_moves:
            ...
            if self.hash in self.transposition_table and self.transposition_table[self.hash][0] >= depth:
                ...
                self.transposition_table[self.hash] = (depth, score)
            self.undo_move()
            if self.board.turn:
                if score > alpha:
                    alpha = score
    ...
    def get_best_move(self, max_depth: int = 3) -> str:
        start = time.time()
        move = self.iterative_deepening(max_depth)
